                ...
                bool allow_lookup, uint16_t port)
{
    ...
    LookupHost(psz_name, &vip, 1, allow_lookup, port);
    if(vip.empty())
        return false;
    *out = vip.front();
...
bool LookupSubNet(const char* psz_name, uint16_t port, SubNet *out)
{
    ...
    if (LookupHost(str_addr.c_str(), &vip, 1, false, port))
    {
